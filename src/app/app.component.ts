...
export class AppComponent implements OnInit {
  ...
  playSoundtrackAudio(): void {
    (this.soundtrackAudio.loop = true), this.soundtrackAudio.load();
  ...
  addHouses(list: Person[]): Person[] {
    ...
    const gryffindorList = listToEdit
      .splice(0, peoplePerHouse)
      .map((item) => addHouse(item, HouseName.gryffindor));
    const hufflepuffList = listToEdit
      .splice(0, peoplePerHouse)
      .map((item) => addHouse(item, HouseName.hufflepuff));
    const ravenclawList = listToEdit
      .splice(0, peoplePerHouse)
      .map((item) => addHouse(item, HouseName.ravenclaw));
    const slytherinList = listToEdit
      .splice(0, peoplePerHouse)
      .map((item) => addHouse(item, HouseName.slytherin));
  ...
  playAudio(audioType: string): void {
    const gryfindorAudioVariationsAmount = 3;
    const hufflepuffAudioVariationsAmount = 3;
    const ravenclawAudioVariationsAmount = 3;
    const slytherinAudioVariationsAmount = 3;
    let audioVariant = '';
    if (audioType === HouseName.gryffindor) {
      audioVariant = this.getRandomNumber(
        1,
        gryfindorAudioVariationsAmount
      ).toString();
    } else if (audioType === HouseName.hufflepuff) {
      audioVariant = this.getRandomNumber(
        1,
        hufflepuffAudioVariationsAmount
      ).toString();
    } else if (audioType === HouseName.ravenclaw) {
      audioVariant = this.getRandomNumber(
        1,
        ravenclawAudioVariationsAmount
      ).toString();
    } else if (audioType === HouseName.slytherin) {
      audioVariant = this.getRandomNumber(
        1,
        slytherinAudioVariationsAmount
      ).toString();
    }
